...
search_with_zoxdie() {
    if [ -z "$1" ]; then
        ...
        file="$(fd --type f -I -H -E .git -E .git-crypt -E .cache -E .backup | sk --height=70% --preview='bat -n --color=always --line-range :500 {}')"
        if [ -n "$file" ]; then
            nvim "$file"
        fi
    else
        # Handle when an arg is provided
        lines=$(zoxide query -l | xargs -I {} fd --type f -I -H -E .git -E .git-crypt -E .cache -E .backup -E .vscode "$1" {} | sk --no-sort) # Initial filter attempt with fzf
